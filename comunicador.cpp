...
int main(int argc, char *argv[])
{
    ...
    MPI_Comm_split(MPI_COMM_WORLD // a partir del comunicador global.
                   ...
                   , rank, // indica el orden de asignacion de rango dentro de los nuevoscomunicadores
    ...
    if(color==1)
    {
        if(rank_nuevo == 0)
        {
            ...
            {
                miArray.push_back(i*5);
            }
        }
        MPI_Scatter(&miArray[0],1,MPI_INT, &numero, 1, MPI_INT, 0, comm);
    }
    cout << "Soy el proceso " << rank << " de " << size << " dentro de MPI_COMM_WORLD,"
         ...
         " aqui he recibido el valor " << b <<"\n"<< "Valor de MPI_Scatter: " << numero << endl;
